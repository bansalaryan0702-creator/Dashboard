...
const getCellText = (row, index) => {
  try {
    ...
    if (!cell) return "";
    return cell.text || (cell.value ? String(cell.value) : "");
  } catch (e) {
    try {
      const cell = row.getCell(index);
  ...
  app.post("/api/catalogue/import-sheets", async (req, res) => {
    try {
      const { url } = req.body;
      ...
      if (!sheetId) return res.status(400).json({ error: "Invalid Google Sheets URL" });
      
      const exportUrl = `https://docs.google.com/spreadsheets/d/${sheetId}/export?format=xlsx`;
      const response = await fetch(exportUrl);
      
      if (!response.ok) {
        return res.status(400).json({ error: "Failed to download sheet. Make sure it is set to 'Anyone with the link can view'." });
      }
      
      const contentType = response.headers.get("content-type") || "";
      if (contentType.includes("text/html")) {
        return res.status(400).json({ error: "Sheet is private. Please change the sharing settings to 'Anyone with the link can view'." });
      }
      ...
      if (!worksheet) return res.status(400).json({ error: "No worksheets found" });

      const items = [];
      worksheet.eachRow((row, rowNumber) => {
          if (rowNumber === 1) return; // skip header
          const brandName = getCellText(row, 1) || "";
          const name = getCellText(row, 2) || "";
          if (!name) return;
          const description = getCellText(row, 3) || "";
          const priceStr = getCellText(row, 4) || "0";
          const category = getCellText(row, 5) || "Uncategorized";
          const sizesStr = getCellText(row, 6) || "";
          
          let sizes = sizesStr.split(',').map(s => s.trim()).filter(Boolean);
      ...
      const db = await readDB();
      if (!db.catalogueItems) db.catalogueItems = [];
      db.catalogueItems = [...db.catalogueItems, ...items];
      
      const newCategories = items.map(i => i.category).filter(Boolean);
      ...
      res.status(500).json({ error: "Server error during Sheets import. " + err.message });
    }
  });

  app.post("/api/catalogue/bulk-import-json", async (req, res) => {
    try {
      ...
      console.error(err);
      res.status(500).json({ error: "Server error during bulk JSON import" });
    }
  });

  app.post("/api/catalogue/upload-excel", upload.single("file"), async (req, res) => {
    try {
      if (!req.file) {
      ...
      for (const image of worksheet.getImages()) {
        ...
        const imgMedia = workbook.getImage(Number(image.imageId));
        imagesByRow[row] = imgMedia;
      }
      ...
      let s3Client = null;
      try {
        s3Client = (bucketName && process.env.AWS_ACCESS_KEY_ID && process.env.AWS_SECRET_ACCESS_KEY) ? getS3Client() : null;
      } catch (err) {
        console.warn("Could not init S3 client:", err.message);
      }
      ...
      worksheet.eachRow((row, rowNumber) => {
        ...
        promises.push((async () => {
          ...
          let imageUrl = "";
          const imgMedia = imagesByRow[rowNumber - 1]; // nativeRow is 0-indexed
          
          if (imgMedia && imgMedia.buffer) {
             const ext = imgMedia.extension || "png";
             if (s3Client && bucketName) {
               const key = `catalogue/excel_${uuidv4()}.${ext}`;
               const command = new PutObjectCommand({
                 Bucket: bucketName,
                 Key: key,
                 Body: imgMedia.buffer,
      ...
      const newCategories = items.map(i => i.category).filter(Boolean);
      if (!db.categories) db.categories = [];
      db.categories = Array.from(new Set([...db.categories, ...newCategories]));
